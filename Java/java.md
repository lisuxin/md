...
```
十进制转换为二进制：除二取余，结果从下向上排列
二进制转换为十进制：从右到左依次序乘以位权
```
...
`jre：Java`运行时环境，包含`jvm`和运行时所需要的核心类库。
...
原料:铁矿石、煤炭
产出物:钢铁建材
参数(原料) :就是进入方法的数据。返回值(产出物) :就是从方法中出来的数据。
定义方法的完整格式:
...
6. PS:参数如果有多个`,`使用逗号进行分隔
7. 方法体:方法需要做的事情,若干行代码
8. return:两个作用,第一停止当前方法,第二将后面的返回值还给调用处
9. 返回值:也就是方法执行后最终产生的数据结果
10. 注意: return后面的“返回值”,必须和方法名称前面的“返回值类型”,保持对应。
10. 方法上有横线说明方法过时了
...
![image-20220523084637005](D:\笔记\typoratuxiang\java\方法调用流程图解.png)
...
`数组名称[索引值]`索引值:就是一个int数字,代表数组当中元素的编号。
...
![image-20220523105614308](D:\笔记\typoratuxiang\java\一个数组的内存图.png)

### 两个数组的内存图

![image-20220523110145678](D:\笔记\typoratuxiang\java\两个数组的内存图.png)
...
成员方法(行为):public void eat() // 吃饭;public void sleep(); // 睡觉public void study() ;//学 习
...
![image-20220523150831365](D:\笔记\typoratuxiang\java\一个对象的内存图.png)
...
用private关键字将需要保护的成员变量进行修饰。一旦使用了private进行修饰,那么本类当中仍然可以随意访问。但是!超出了本类范围之外就不能再直接访问了。

间接访问private成员变量,就是定义一对儿Getter/Setter方法

必须叫setxxx或者是getXxx命名规则。对于Getter来说,不能有参数,返回值类型和成员变量对应;对于Setter来说,不能有返回值,参数类型和成员变量对应。 
...
* String str4 = "Hello"注意:直接写上双引号,就是字符串对象。
...
![image-20220525231542164](D:\笔记\typoratuxiang\java\字符串常量池.png)
...
* `public boolean equalsIgnoreCase(String str)`:忽略大小写,进行内容比较。只有英文字母区分大小写。
...
![image-20220526083823624](D:\笔记\typoratuxiang\java\Static.png)
...
public class Student02 {
    public static void main(String[] args) {
        Student01 stu1 = new Student01("哈哈",20);
        stu1.room="102";
        System.out.println("姓名:"+stu1.getName()+",年龄:"+stu1.getAge()+",教室:"+Student.room+",学号:"+stu1.getId());

        Student01 stu2 = new Student01("嘿嘿",21);
        System.out.println("姓名:"+stu2.getName()+",年龄:"+stu2.getAge()+",教室:"+Student.room+",学号:"+stu2.getId());
    }
...
public class Student01 {
    ...
    public static int getIdConter() {
        return idConter;
    }

    public static void setIdConter(int idConter) {
        Student01.idConter = idConter;
    }
}
```
...
![image-20220526094623634](D:\笔记\typoratuxiang\java\static内存图.png)
...
修饰符权限大小关系：public > protected > (default) > private
...
在第三种用法当中要注意,this(...)调用也必须是构造方法的第一个语句。

**this和super关键字图解**

![image-20220527004105496](D:\笔记\typoratuxiang\java\thissuper.png)
...
public class Yonghu {
    ...
    public Yonghu(String name,int yue) {
        this.name = name;
        this.yue = yue;
    }
    ...
    //展示当前用户有多少钱
    public void show() {
        System.out.println("我是" + name + ";我有：" + yue + "元");
    }
...
public class Fa extends Yonghu{
    ...
    public Fa(String name, int yue) {
        super(name, yue);
    }

    public ArrayList<Integer> send(int totalMonye,int count){
        //首先需要一个集合,用来存储若干个红包的金额
        ArrayList<Integer> redList= new ArrayList<>();
        //首先看一下群主自己有多少钱
        int leftMoney = super. getYue(); // 群主当前余額
        if (totalMonye > leftMoney) {
            System.out.println("余额不足");
            return redList;//返回空集合
        }
        //从新设置余额，扣钱
        super.setYue(leftMoney - totalMonye);
        int avg = totalMonye / count;
        int mod = totalMonye % count;//求余数
...
public class Shou extends Yonghu{
    ...
    public Shou(String name, int yue) {
        super(name, yue);
    }
    ...
    public void receive(ArrayList<Integer> list){
        ...
        int money = super.getYue();
        //加法从新设置回去
        super.setYue(money + delta);;
    }
}
```
...
public class ShiXian {
    public static void main(String[] args) {
        Fa shou = new Fa("老大",100);
        Shou two = new Shou("老三",100);
        Shou one = new Shou("老二",100);
        Shou three = new Shou("老四",100);
        shou.show();
        one.show();
        two.show();
        three.show();
        System.out.println("================");
        ArrayList<Integer> arrayList = shou.send(20,3);
...
`类似于:int num = (int) 10.0: // 可以int num = (int) 10.5;//不可以,精度损失`
