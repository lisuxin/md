...
| Ctrl + D                 | 复制并换行                                      | Ctrl + Y     | 删除当前行                   |
| Ctrl + Alt + 空格        | 代码提示（类似于 Alt + /）                      | Alt + INSERT | 生成 getter 和 setter 的方法 |
| Ctrl + Alt + T           | 选中某一个段代码进行 try_catch / if / while ... | Alt + 回车   | 导包 / 引入局部变量          |
| Ctrl + B / Ctrl 单击鼠标 | 查看类的源码，和 Eclipse 的用法是一样           | Shift + F6   | 重命名                       |
...
**12）Ctrl + Shift + N 搜索文件**

**13）Alt + F7 查询在某处使用**

**14）Ctrl + Q 查看代码提示及内容**

**15）Ctrl + F12 查看文件结构（File Structure）**

**16）Ctrl + Shift + 空格 在new关键字之后使用智能类型代码完成来实例化期望类型的对象**

**17）Ctrl + Alt + B 查看接口或者抽象类的子类。**

**18）get,set方法快捷键ALT+INSERT**

Ctrl+A：全选                        Ctrl+Z：撤销                        Ctrl+X：剪切

Ctrl+C：复制                        Ctrl+V：粘贴                        Ctrl+Y：删除当前行

Ctrl+D：复制当前行到下一行            Ctrl+O：选择可重写的方法             Ctrl+I：重写接口方法

Ctrl+/：快速单行注释                  Ctrl+Shift+/：快速多行注释             Ctrl+Alt+V：快速赋值            

Ctrl+Shift+L：快速格式化代码           Ctrl+Shift+F10：快速运行程序          Ctrl+Shift+Alt+Insert：快速新建文件

Ctrl不放：移动鼠标到相应的位置可以查看相关的东西

Shift+Home：光标移动到行首           Shift+End：光标移动到行尾             Shift+PgUp：光标移动到页首

Shift+PgDn：光标移动到页尾            Shift+上/下：快速选择上下行

Alt+Enter：快速强制转换               Alt+左/右：快速切换文件               Alt+Insert+Shift：快速创建构造函数

Alt+Shift+上/下：将当前行内容上/下移动

切换java程序:Alt + 右箭头 或 Alt + 左箭头

七. 目录显示与关闭:Ctrl + Fn + F12
